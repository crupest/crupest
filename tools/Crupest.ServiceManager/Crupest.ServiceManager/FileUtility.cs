...
namespace Crupest.ServiceManager;
...
public static partial class FileUtility
{
    ...
    // Only recognize variable in ${NAME} form. (Must contain brackets)
    public static string TextFromTemplate(string template, Dictionary<string, string> dict, bool allowUnknownKey = true)
    {
        return TemplateValuePattern.Replace(template, (match) =>
        {
            ...
            if (!allowUnknownKey)
            {
                throw new Exception($"Unknown key '{key}' in template.");
            }
            return match.Value;
        });
    }
