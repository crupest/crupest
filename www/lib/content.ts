...
/**
 * Converts HTML to plain text using jsdom's textContent.
 */
...
function extractSummary(renderedHtml: string, plainText: string): string {
  ...
  if (moreIndex !== -1) {
    ...
    return htmlToPlainText(preMoreHtml);
  }
...
const clipboardSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard" viewBox="0 0 16 16">
...
async function loadArticleFile(
  ...
  const { data: rawFrontmatter, content: body } = matter(raw);
      ...
      if (token.type === "code") {
        ...
        token.text = `
          <div id="code-block-${codeBlockCount}" class="code-block">
            <div><span>${token.lang ?? "plain"}</span><span
                class="copy-button"
                onclick="copyCodeBlock(${codeBlockCount})">${clipboardSvg}</span></div>
            ${token.text}
          </div>
        `;
        codeBlockCount++;
