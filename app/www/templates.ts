import { html, type Html, raw } from "@crupest/base-contrib/html";
...
function navHtml(path: string, site: Site): Html {
  ...
  const links = crumbs
    .slice(0, -1)
    .map(
      (c) => html` <a class="mono-link" href="${c.href}">${c.name}</a> &gt; `,
    );
  return html` <nav class="mono">${links}this</nav> `;
}
...
function articlePreviewHtml(
  ...
  return html`
    <section class="article-preview">
      ...
      ${raw(`<${headingTag} class="title">`)}<a
        href="${site.pageLink(article.path)}"
        >${article.title}</a
      >${raw(`</${headingTag}>`)}
      <p class="content">
      ...
      <p>
        ...
        <a class="mono-link" href="${site.pageLink(article.path)}">Read more</a>
      </p>
    </section>
  `;
}
...
function articlePreviewListHtml(
  ...
  return html`
    ${articles.map(
      (article, i) => html`
        ${i > 0 && html` <hr class="article-preview-hr" /> `}
      ...
      content: html`
        ...
        <section>
          ...
          <p>
            ...
            <a href="mailto:crupest@crupest.life">crupest@crupest.life</a>, or
            just create an issue in any of my
            <a rel="noopener noreferrer" href="https://github.com/crupest"
              >GitHub</a
            >
          ...
          <div id="links" class="mono-link">
            ...
            <ul>
              ...
              <li>
                <a href="${site.pageLink("/notes/cheat-sheet/")}"
                  >cheat-sheet</a
                >
              </li>
            </ul>
        ...
        <section id="recent-posts">
          <h2>
            Recent Posts
            <a class="mono-link" href="${site.pageLink("/posts/")}">(all)</a>
          </h2>
  ...
  return html`
    <div class="friend">
      ...
      <a rel="noopener noreferrer" href="${ghUrl}">
        <img
          class="friend-github"
          src="${site.resourceLink.asset("gh.png")}"
          alt="github logo"
        /> </a
      ><br />
      ${friend.tag && html` <span class="friend-tag">${friend.tag}</span> `}
    </div>
  `;
}
  ...
  return baseLayout(
    {
      ...
      content: html`
        ...
        <hr />
        <p class="post-info">
          ${html` <span class="created">${dateHtml(article.date)}</span> `} |
          ...
          ${
            ...
            html`
              <span class="last-updated"
                >Last updated: ${dateHtml(article.lastmod)}</span
              >
            `
          }
        </p>
