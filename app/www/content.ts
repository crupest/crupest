...
async function loadArticle(dir: string, sourcePath: string): Promise<Article> {
      ...
      if (token.type === "code") {
        ...
        token.text = html`
          <div id="code-block-${codeBlockCount}" class="code-block">
            <div>
              <span>${token.lang ?? "plain"}</span
              ><span
                class="copy-button"
                onclick="copyCodeBlock(${codeBlockCount})"
                >${clipboardSvg}</span
              >
            </div>
            ${html.raw(token.text)}
  ...
  const plainText = new JSDOM(renderedHtml).window.document.body.textContent;
...
export async function loadArticles(dir: string): Promise<Article[]> {
  ...
  for (const entry of entries) {
    const article = await loadArticle(dir, "/" + entry.replaceAll("\\", "/"));
