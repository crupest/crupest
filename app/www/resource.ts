...
export class Resource {
  ...
  get mimeType(): string {
    ...
    if (extension.length === 0) return fallback;
  ...
  async getContent(): Promise<Uint8Array> {
    if (this.#content == null) {
      if (this.#source.type === "data") {
        ...
        this.#content = await readFile(this.#source.path);
      }
    }
    return this.#content;
  }
  ...
  async writeContent(root: string | null): Promise<void> {
    ...
    await writeFile(fullPath, await this.getContent());
  }
...
export async function scanResources(dir: string): Promise<Resource[]> {
  ...
  for (const entry of entries) {
    const relativePath =
      "/" + entry.slice(dir.length + 1).replaceAll("\\", "/");
    const resource = new Resource({
      source: { type: "file", path: entry },
      path: relativePath,
