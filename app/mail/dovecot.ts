...
import { type ILogger, NULL_LOGGER } from "@crupest/base/log";

import { type Mail, type MailDeliverContext, MailDeliverer } from "./mail.js";
...
async function runCommand(
  ...
  try {
    ...
    if (errorCodeMessageMap?.has(status.code)) {
    ...
    const message =
      "A JS error was thrown when invoking external command " + cause;
...
export class DovecotMailDeliverer extends MailDeliverer {
  ...
  protected override async doDeliver(
    ...
    for (const recipient of recipients) {
      const result = await runCommand(this.#ldaPath, {
        logger: NULL_LOGGER,
        args: ["-d", recipient],
        stdin: utf8Bytes,
  ...
  async saveNewSent(logger: ILogger, mail: Mail, messageIdToDelete: string) {
    ...
    if (messageId != null) {
      ...
      logger.warn("Message id of the mail is not found, skip marking as read.");
    }
