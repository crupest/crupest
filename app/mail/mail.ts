...
import { simpleParseMail } from "./mail-parsing.js";
...
export class Mail {
  ...
  toBase64(): string {
    return Buffer.from(this.raw, "utf8").toString("base64");
  }
...
export class MailDeliverResult {
  ...
  generateMessageForSmtp(): string {
    ...
    return `2.0.0 OK${StringUtils.prependNonEmpty(
      this.newMessageId,
...
export class RecipientFromHeadersHook implements MailDeliverHook {
  ...
  callback(context: MailDeliverContext) {
    if (context.recipients.size !== 0) {
      ...
      [...context.mail.parsed.recipients]
        .filter((r) => r.endsWith("@" + this.mailDomain))
...
export class AliasRecipientMailHook implements MailDeliverHook {
  ...
  async #parseAliasFile(
    ...
    try {
      if ((await stat(this.#aliasFile)).isFile()) {
        const text = await readFile(this.#aliasFile, "utf8");
