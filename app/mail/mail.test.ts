...
import { Mail, type MailDeliverContext, MailDeliverer } from "./mail.js";
...
describe("Mail", () => {
  ...
  it("simple parse date", () => {
    expect(new Mail(mockMailStr, NULL_LOGGER).parsed.date).toEqual(
      new Date(mockDate),
    );
  });

  it("simple parse headers", () => {
    expect(new Mail(mockMailStr, NULL_LOGGER).parsed.headers).toEqual(
      mockHeaders.map((h) => [h[0], " " + h[1].replaceAll("\n", "")]),
    );
  });
...
describe("MailDeliverer", () => {
  class MockMailDeliverer extends MailDeliverer {
    name = "mock";
    override doDeliver = vi.fn((_: Mail, ctx: MailDeliverContext) => {
