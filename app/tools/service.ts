...
import { defineYargsModule, DEMAND_COMMAND_MESSAGE } from "./yargs.js";
...
function loadTemplatedConfigFiles(files: string[]): Record<string, string> {
...
class TemplateDir {
  ...
  allNeededVars() {
    return [...new Set(this.templates.flatMap((t) => t.vars))];
  }

  generate(vars: Record<string, string>, generatedDir?: string) {
    console.log(`Generate to dir ${generatedDir ?? "[dry-run]"} ...`);
    ...
    return builder
      ...
      .command({
        ...
        handler: (argv) => {
          ...
          new TemplateDir(join(projectDir, "services/templates")).generate(
            config,
